        ...
        <div className="flex flex-col h-full min-h-0 overflow-hidden">
            ...
            <div className="flex-1 min-h-0 flex flex-col transition-all duration-500">
                {!showBottom ? (
                    ...
                    <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-8 duration-500 items-stretch">
            ...
            <div className="flex flex-col items-center pt-4 shrink-0">
                <div className="w-full h-px bg-slate-200 mb-4 opacity-50" />
                <button
                    onClick={() => setShowBottom(!showBottom)}
                    className="flex items-center gap-3 bg-primary text-white border-2 border-white/20 px-12 py-3 rounded-full font-black text-xs uppercase tracking-[0.2em] shadow-2xl hover:bg-primary-light hover:scale-105 transition-all active:scale-95 group"
                >
                    <Layers size={18} className={showBottom ? 'animate-pulse' : ''} />
                    <span>{showBottom ? 'Voltar para o Mapa de Calor' : 'Ver Ranking e Unidade em Destaque'}</span>
